...
describe("Utils", function () {
    ...
    describe("Objects", function () {
        ...
        it('should merge ', async () => {
            ...
            Util.objects.merge(object, other,{c:2}).should.be.deep.equals({'a': [{ 'b': 2, 'c': 3 }, { 'd': 4, 'e': 5 }], 'c':2});
            Util.objects.merge(object, other,{a:[{c:2}]}).should.be.deep.equals({'a': [{ 'b': 2, 'c': 2 }, { 'd': 4, 'e': 5 }], 'c':2});
            Util.objects.merge({'a': {'b': {'c': 2}}}, {'a': {'b': {'d': 4}}}).should.be.deep.equals({'a': {'b': {'c': 2,'d': 4}}});

            Util.objects.merge({'a': [{ 'b': 2 }, { 'd': 4 }]}, {'a': [{ 'c': 3 }, { 'e': 5 }]}).should.be.deep.equals({'a': [{ 'b': 2, 'c': 3 }, { 'd': 4, 'e': 5 }]});

           Util.objects.merge({'a':{b: {c:[2,3]}}}, {'a':{b: {d:[3,4]}}}).should.be.deep.equals({'a':{b: {c:[2,3],d:[3,4]}}});


            Util.objects.merge({'a': {'b': {'c': 1, 'd': [2, 3]},'e': [4, 5]},'f': 6},  {'a': {'b': {'c': 7, 'd': [8, 9]}, 'e': [10, 11]}, 'f': 12, 'g': {'h': 13}})
